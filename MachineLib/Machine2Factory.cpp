...
Machine2Factory::Machine2Factory(const std::wstring resourcesDir)
...
std::shared_ptr<Machine> Machine2Factory::Create()
{
    ...
    std::cout << "The number is: " << mImagesDir << std::endl;
    ...
    upright->SetColor(*wxRED);

    // center it on the 250px platform:
    int centerX = platformX + 125;
    upright->SetLocation(wxPoint(centerX -150, platformY - 27));
    ...
    blower->SetLocation(wxPoint(-158, -299));


    blower->SetBlowLeft(true);
